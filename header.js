...
Richard Vera, domain_disabled, Now
document.addEventListener('DOMContentLoaded', () => {
  // ---------------- Floating Header ----------------
  const header = document.createElement('div');
  header.style.position = 'fixed';
  ...
  header.style.left = '0';
  header.style.width = '100%';
  header.style.height = '60px'; // compact height
  header.style.zIndex = '9999';
  header.style.display = 'flex';
  header.style.alignItems = 'center';
  header.style.justifyContent = 'space-between';
  ...
  logo.alt = 'Logo';
  logo.style.height = '40px'; // smaller
  header.appendChild(logo);

  // Right section: clocks + username
  ...
  const digital = document.createElement('div');
  digital.id = 'digitalTime';
  digital.style.fontSize = '20px'; // readable but compact
  digital.style.fontFamily = 'monospace';
  ...
  // Adjust body padding to fit header
  ...
  function drawTime(ctx, radius) {
    ...
    drawHand(ctx, (hour + minute / 60 + second / 3600) * Math.PI / 6, radius * 0.45, 3);
    drawHand(ctx, (minute + second / 60) * Math.PI / 30, radius * 0.65, 2);
    drawHand(ctx, second * Math.PI / 30, radius * 0.75, 1, 'red');
  }
